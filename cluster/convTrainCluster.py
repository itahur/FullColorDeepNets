...
def trainNet(dataset_std, dataset_size, model_type, model_layers1, model_layers2, features, n_exp, num_epochs, mix, freeze_weights):    
    dataset_filename = ""
    if dataset_std == 0:
    ...
    with open("/tudelft.net/staff-umbrella/StudentsCVlab/itahur/custom_datasets/" + str(dataset_size) + "/" + dataset_filename + ".pkl", "rb") as f:
        ...
        for iteration in tqdm(range(2, 13, 2), total=6, desc=f"ds std: {dataset_std}, ds size: {dataset_size}"):
            ...
            # Model layers
            model_layers = model_layers1 if model_type == 1 else model_layers2
            # Set seed
            torch.manual_seed(iteration)
            # Create model
            model = separateChannel3DNet(model_layers, model_layers[-2], features, mix) if model_type == 1 else twoChannel3DNet(model_layers, model_layers[-2], features, mix)
            # Load file and save file
            # lfile = "Gaussian3D_" + sfile_fill
            sfile = str(model_type) + "-Gaussian3D_" + sfile_fill
            # Load model
            ...
            with open('Experiments/conv/trainvalAccs/' + str(n_exp) + '/' + str(dataset_size) + '/' + str(iteration) + '/' + sfile + '.txt', 'w') as f:
            ...
            # Save the model
            torch.save(model.state_dict(), 'Experiments/conv/model_saves/' + str(n_exp) + '/' + str(dataset_size) + '/' + str(iteration) + '/' + sfile + '.pth')
            
def trainNets(dataset_stds, dataset_sizes, model_type, model_layers1, model_layers2, features, n_exp, num_epochs=20, mix=False, freeze_weights=False):
    if not os.path.exists('./Experiments/conv/model_saves/' + str(n_exp)):
        create_dirs('./Experiments/conv/model_saves/' + str(n_exp) + '/', dataset_sizes)
    if not os.path.exists('./Experiments/conv/trainvalAccs/' + str(n_exp)):
        create_dirs('./Experiments/conv/trainvalAccs/' + str(n_exp) + '/', dataset_sizes)
        
    for size in dataset_sizes:
        for std in dataset_stds:
            trainNet(std, size, model_type, model_layers1, model_layers2, features, n_exp, num_epochs, mix, freeze_weights)